...
internal class Program
{
    ...
    private static async Task Main(string[] args)
    {
        ...
        using (var httpClientV4 = new HttpClient() { BaseAddress = new Uri(environmentUrl) })
        {
            ...
            httpClientV4.DefaultRequestHeaders.Add("X-API-Key", token);

            await CallAccountAsync(httpClientV3);
            ...
            await CreateBuildingFileAsync(httpClientV3, buildingId, productId);

            await CreateMaterialPassportAsync(httpClientV3, buildingId);
        }
    }
    ...
    /// </summary>
    private static async Task CallAccountAsync(HttpClient httpClient)
    {
    ...
    /// </summary>
    private static async Task<Guid?> GetFolderTypeIdAsync(HttpClient httpClient)
    {
        var client = new AccountClient(httpClient);
    ...
    /// </summary>
    private static async Task<Guid> GetOrCreateFolderAsync(HttpClient httpClient, Guid folderTypeId)
    {
    ...
    /// </summary>
    private static async Task<Guid> CreateBuildingAsync(HttpClient httpClient, Guid folderId)
    {
        var client = new BuildingClient(httpClient);
        var settingsClient = new V3.SystemSettingsClient(httpClient);
        ...
        var building = await client.AddBuildingAsync(new BuildingRequest()
        {
            ...
            PreparedBy = "Example Architects",
            OwnerName = "Building Owner",
            OwnerAddressLine1 = "Amersfoortsestraatweg 117",
            OwnerZipcode = "1251 AV",
            OwnerCity = "Laren",
            OwnerCountryId = "NL"
        });
    ...
    /// </summary>
    private static async Task CreateBuildingFileAsync(HttpClient httpClient, Guid buildingId, Guid productId)
    {
    ...
    /// </summary>
    private static async Task CreateMaterialPassportAsync(HttpClient httpClient, Guid buildingId)
    {
        var fileClient = new BuildingFileClient(httpClient);
        ...
        var pdf = await buildingClient.CreatePassportAsync(buildingId, new PassportRequest()
        {
            PassportType = PassportType.Full,
            Language = "nl",
            Preset = Guid.Empty
        });
